File
    ...
    @open(mode:OpenMode)
        file = File(.filePath, getNativeOpenMode(mode))
    
    getNativeOpenMode(mode:OpenMode):String
        if (mode == OpenMode.READ) || _
...
Button -> Control
    ...
    @mouseUp:CallBack<MouseUpEvent, Control>
...
MainFrame -> Frame
    ...
    @mouseUpDeleteButton(event:MouseUpEvent, sender:Control)
        result:DialogResult = .showMessage("Can I delete it?", _
                                           "Confirm", _
                                           Icon.QUESTION, _
    ...
    @shown()
        file:File = File("./data.txt", OpenMode.READ)
        entries:[]<Entry> = []<Entry>
        lines:[]<String> = file.getAllLine()
